...
class StockDataService:
    ...
    @staticmethod
    def get_historical_prices(stock, period="1y"):
    ...
    @staticmethod
    def get_profit_and_margin_history(stock):
    ...
    @staticmethod
    def compute_piotroski_fscore(stock):
        ...
        if bs.empty or is_.empty or not info:
            return {"total_score":0,"profitability":[],"leverage":[],"operating":[],"interpretation":"N/A"}

        def safe(df,row,col):
            try: return StockDataService.safe_float(df.at[row,col])
        ...
        # Interprétation
        if total_score>=7: interp="Entreprise solide. Très bonne gestion"
        elif total_score>=4: interp="Entreprise avec du potentiel. Bien analyser avant d'investir"
        else: interp="A fuir"

        return {"total_score":total_score,"profitability":profitability,"leverage":leverage,"operating":operating,"interpretation":interp}
